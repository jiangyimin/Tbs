...
using Tbs.Web.Models.Account;
using Tbs.Authorization.Users;
using Tbs.Controllers;
using Tbs.Identity;
using Tbs.Sessions;
using Tbs.Web.Views.Shared.Components.TenantChange;
...
namespace Tbs.Web.Controllers
{
    public class AccountController : TbsControllerBase
    {
        ...
        {
            ValidIpAddress(loginModel.UsernameOrEmailAddress, GetTenancyNameOrNull());
        ...
        private void ValidIpAddress(string userName, string tenancyName)
        {
            if (userName == "admin") return;

            var iplist = SettingManager.GetSettingValueForTenant(SettingNames.WorkFlow.LoginIpList, AbpSession.TenantId.Value);
